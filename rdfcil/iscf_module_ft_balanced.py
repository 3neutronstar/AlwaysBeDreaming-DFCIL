...
class ISCFModuleFTB(FeatureHookMixin, FinetuningMixin, cl.Module):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.model_old is not None:
            ...
            if self.finetuning:
                cur_classes = n_cur - n_old
                old_classes = n_old
                bsz = 2 * input.shape[0]
                num_old_samples = floor(old_classes / (cur_classes + old_classes) * bsz)
                num_cur_samples = bsz - num_old_samples
                if input_rh.shape[0] < num_old_samples:
                    sup_input_rh, sup_target_rh = self.inversion.sample(num_old_samples)
                    input_rh = torch.cat([input_rh, sup_input_rh], dim=0)
                    target_rh = torch.cat([target_rh, sup_target_rh], dim=0)
                    input_rh= input_rh[:num_old_samples]
                    target_rh= target_rh[:num_old_samples]
                    input = input [:num_cur_samples]
                    target_t = target_t[:num_cur_samples]
                else:
                    # input_rh = input_rh[:num_old_samples]
                    # target_rh = target_rh[:num_old_samples]
                    # input = input[:num_cur_samples]
