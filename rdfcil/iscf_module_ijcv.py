...
class ISCFModule(FeatureHookMixin, FinetuningMixin, cl.Module):
    ...
    def register_losses(self):
        ...
        self.register_loss(
            ...
            self.hparams.lambda_ft * self.alpha * self.beta
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.model_old is not None:
            ...
            # hkd
            loss_kd=(F.mse_loss(outputs[:,:n_old],self.model_old.head(old_z).detach(),reduction='none').sum(dim=1))*self.hparams.lambda_hkd * self.alpha * self.beta
            ...
            loss_weq=F.mse_loss(last_params.norm(dim=1,keepdim=True).mean().view(-1).expand(n_cur),cur_params.norm(dim=1)) *self.hparams.lambda_weq * self.alpha * self.beta
